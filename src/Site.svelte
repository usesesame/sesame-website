<script lang="ts">
  ...
  const repositories = [
    ...
    { name: 'sesame-browser-extension', what: 'The Chrome, Edge, and Firefox extension, packaged reproducibly.' },
  ]
...
<main id="top">
  ...
  {:else if page === 'roadmap'}
    ...
    <section class="section document-section roadmap-section">
      ...
      <article class="document-copy">
        ...
        <div id="browser"><h2>Browser extension</h2><p>The extension uses native messaging rather than a local server. It detects supported sign-in fields, handles multi-step and single-page sign-in flows, and fills only after a user action and desktop approval. It also offers saved cards on HTTPS checkout forms, including fields held in Stripe payment frames, under the same per-fill approval. It never submits the page and never writes a credential to extension storage. The inline field control is optional, stays off until you grant host permission, and can be paused per origin or revoked at any time.</p><p>Reproducible Chrome, Edge, and Firefox upload archives build today with recorded digests and a fixed extension identity the native host manifest must match. Before release it still needs store submission, a signed desktop installer that registers, upgrades, and removes the native host, and clean-profile install and removal verification.</p></div>
  ...
  {:else if page === 'releases'}
    ...
    <section class="section release-section">
      ...
      {:else}
        <div class="release-requirements"><h2>Nothing to download yet</h2><p>No Windows build has cleared the release gate. When one does it appears here, with its SHA-256 and updater signature, and no account is needed to fetch it.</p></div>
      {/if}
  ...
  {:else if page === 'terms'}
    <section class="page-hero compact-page-hero legal-hero">
      <h1>Terms of use.</h1>
      <p class="intro">These terms govern your use of the Sesame website and the hosted account service. By using them, you agree to what follows.</p>

...
<section class="section document-section roadmap-section">
  ...
  <article class="document-copy">
    <div id="today" use:reveal><h2>Available to test</h2><p>The Windows and Linux app stores logins in a local vault. You can import records, search them, check passwords, and create or restore backups.</p></div>
    <div id="backups" use:reveal><h2>Backup compatibility</h2><p>Conversion for older backups is implemented. Every official release proves restore and restart against the historical backup corpus on Windows and Linux before publication. Keep the original backup file.</p></div>
    <div id="gates" use:reveal><h2>Release checks</h2><p>Every official release runs the same checks before publication: the pipeline installs the packaged app, restores every supported historical backup, restarts, and backs up again, on Windows and Linux. Signing Windows installers is still planned.</p></div>
    ...
    <div id="browser" use:reveal><h2>Browser extension</h2><p>A Chrome package and an experimental Firefox package build from source. Store submission, signed native-host installation, and clean-profile lifecycle checks remain before public distribution.</p></div>

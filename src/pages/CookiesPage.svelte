...
<section class="section document-section">
  ...
  <article class="document-copy">
    <div id="about" use:reveal><h2>About cookies</h2><p>Cookies are small files a website stores in your browser. Sesame uses only security and sign-in cookies that are necessary for an account action you request. Public product pages do not set them. Because there are no optional cookies, Sesame does not show a consent banner or save a cookie-banner preference.</p></div>
    <div id="used" use:reveal><h2>Cookies we use</h2>
    ...
    <div id="consent" use:reveal><h2>Consent and choices</h2><p>These cookies are limited to the account action you requested. If Sesame adds a non-essential cookie or similar browser storage, it will request opt-in consent before setting it and provide an equally clear way to withdraw it.</p></div>
    <div id="manage" use:reveal><h2>Managing cookies</h2><p>You can delete or block these cookies in your browser settings. Doing so can prevent sign-in, passkey use, account changes, or support submissions from working, but it does not affect the desktop app or a local vault.</p><p class="doc-meta">Version {LEGAL_VERSION} · Updated {LEGAL_UPDATED}.</p></div>
  </article>
</section>

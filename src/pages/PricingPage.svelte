...
<section class="page-hero compact-page-hero">
  ...
  <p class="intro">Hosted Sync is planned as an optional subscription. Sync is not available yet.</p>
</section>
  ...
  <div class="price-grid">
    {#each productState.plans as plan (plan.id ?? plan.name)}
      <article class="card price-card" class:featured={plan.id === 'free'} use:reveal>
        <p class="price-name">{plan.name} <span class="price-tag" class:current={plan.id === 'free'}>{plan.id === 'free' ? 'Free' : 'Planned'}</span></p>

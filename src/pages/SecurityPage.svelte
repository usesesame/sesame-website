  ...
  <article class="document-copy">
    ...
    <div id="protection" use:reveal>
      ...
      <dl class="hardening-facts">
        ...
        <div><dt>Storage limit</dt><dd>A save above the 64 MiB limit is refused, and the existing vault stays unchanged</dd></div>
      </dl>
      ...
      <p>The unlocked vault key does not sit in ordinary memory. On Windows it is locked out of the page file, re-encrypted when idle, and reachable only through one guarded call. On Linux the kernel locks it against swap and crash dumps, and Sesame wipes it when the kernel refuses that protection. On other systems it is wiped after use.</p>
    </div>
    <div id="browser" use:reveal>
